...
def main():  
    ...
    # train classifier
    nbclf=MultinomialNB(alpha=alphasmooth)
    nbclf.fit(xtrainCounts,ytrain)
    #prediction
    ytestpred=nbclf.predict(xtestCounts)
    trainAccuracy=accuracy_score(ytrain,nbclf.predict(xtrainCounts))
    ...
    confidence=dict(zip(labels, conf_mat.diagonal()/conf_mat.sum(axis=0)))
