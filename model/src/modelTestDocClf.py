...
class TestDocClf(unittest.TestCase):
    def setUp(self):
        self.myModel=DocClf()
        self.loaded=self.myModel.load('C:/Users/jgharris/DocClass/test/v1')
    def testLoaded(self):
        if(not(self.loaded==None)):
            print(self.loaded)
            return
    def testPredict(self):
        ...
        correctConf=0.8308
        output=self.myModel.predictOne(xtest)
        print(output)
        assert(output[0]==correctresult)
        self.assertAlmostEqual(output[1],correctConf,delta=.001)
